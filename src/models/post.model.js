...
const selectAll = async ({ groupedByAuthor = false, page = 0, limit = 0 }) => {
  let [result] = await db.query(
    `
        SELECT t1.title, t1.description, t1.created_at, t1.category, 
...
const selectByTitle = async (title) => {
  const [result] = await db.query("SELECT id FROM post WHERE title = ?", [
    title,
  ]);
  return result;
};
...
const insert = async ({ title, description, category, author_id }) => {
  ...
  const [result] = await db.query(
    `
        INSERT INTO post 
        (title, description, created_at, category, author_id) 
        VALUES (?, ?, ?, ?, ?)
        `,
    [title, description, created_at, category, author_id]
  );
  ...
  return {
    post: { title, description, created_at, category, author_id },
    result,
  };
};
