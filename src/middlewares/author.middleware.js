...
const checkAuthorExists = (req, res, next) => {
    ...
    const [ result ] = db.query('SELECT id FROM author WHERE id = ?', [ author_id ]);
...
const checkInsert = (req, res, next) => {
    ...
    const [ result ] = db.query('SELECT email FROM author WHERE email = ?', [ email ]);
